...
void enregistrer_energie_densite(FILE *fp, double densite, double energie_moyenne){
    fprintf(fp, "%f : %f\n", densite ,energie_moyenne);
}
...
int main() 
{
    ...
    double* rho_variations = linspace(0.1, 1.3, taille);
    double volume_variations[taille];
    double U_tail_variations[taille]; 
    double P_tail_variations[taille]; 
    double L_variations[taille];
    double L_moitie[taille]; 


    for (int s=0; s<taille; s++){

        volume_variations[s]= nbx_particules/rho_variations[s];
        L_variations[s] = sqrt(volume_variations[s]);
        L_moitie[s] = L_variations[s]*0.5; 
        U_tail_variations[s]= pi * epsilon * nbx_particules * rho_variations[s] * (terme_commun_frac * inv_Rc_10 - inv_Rc_4);
        P_tail_variations[s] = 6*pi*epsilon*rho_variations[s]*rho_variations[s]*(terme_commun_frac*inv_Rc_10 - 0.5*inv_Rc_4);
    }
    ...
    FILE *energie_densite_file = fopen("energie_densite.txt","w"); 

    for (int i=0; i<taille; i++){

        int taille_bloc=10; 
        double somme_energie=0.0; 
        ...
        for (int cycle=0 ; cycle<nombre_cycles; cycle++){
            ...
            if (cycle>=(nombre_cycles-1-taille_bloc)){
                somme_energie += calcul_energie_potentielle_terme_commun(tab_particules, index_modidi, L_moitie[i], L_variations[i]) + U_tail_variations[i] + calcul_energie_potentielle_terme_modifie(tab_particules, index_modidi, L_moitie[i], L_variations[i]); 
            }

        }

        double densite = nbx_particules/(volume_variations[i]); 
        double energie_moyenne_part = (somme_energie/taille_bloc)/nbx_particules; 

        enregistrer_energie_densite(energie_densite_file, densite, energie_moyenne_part);

    }
    fclose(energie_densite_file);
